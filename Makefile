...
backend-dev:
	cd backend && if [ -f .env ]; then set -a; . .env; set +a; fi && venv/bin/uvicorn src.main:app --reload --host 0.0.0.0 --port $${PORT:-8000}
...
frontend-dev:
	cd frontend && if [ -f .env.local ]; then set -a; . .env.local; set +a; fi && pnpm dev --port $${PORT:-3000}
